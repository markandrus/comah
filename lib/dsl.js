...
function beforeEach(doneable) {
  var node = focus();
  var describe = null;
  if (node && (describe = node.getValue()) instanceof Describe) {
    return refocus(new Node(describe.beforeEach(doneable), node.getChildren()));
  }
...
function afterEach(doneable) {
  ...
  var describe = null;
  if (node && (describe = node.getValue()) instanceof Describe) {
    return refocus(new Node(describe.afterEach(doneable), node.getChildren()));
  }
...
function timeout(milliseconds) {
  var node = focus();
  if (node) {
    return refocus(new Node(node.getValue().timeout(milliseconds),
      node.getChildren()));
  }
